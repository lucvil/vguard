...
for i in $(seq 0 $((PROPOSER_NUM - 1))); do
  ./scripts/bypass_route_run.sh $i 0 $PARTICIPANT_NUM $MESSAGE_SIZE $NETWORK_DELAY $VEHICLE_SPEED $PROPOSER_LIST $COM_POSSIBILITY_FLAG $ALLOW_BYPASS_FLAG &
  pids+=($!)  # プロセスIDを配列に追加
  sleep 0.5
done
...
for i in $(seq $PROPOSER_NUM $((PARTICIPANT_NUM - 1))); do
  ./scripts/bypass_route_run.sh $i 1 $PARTICIPANT_NUM $MESSAGE_SIZE $NETWORK_DELAY $VEHICLE_SPEED $PROPOSER_LIST $COM_POSSIBILITY_FLAG $ALLOW_BYPASS_FLAG & pid=$!
